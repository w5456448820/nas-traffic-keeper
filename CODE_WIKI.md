# Traffic Keeper - Code Wiki

## 项目概述

**项目名称**: Traffic Keeper - NAS 流量平衡脚本
**版本**: 2.6.16
**平台**: 飞牛 NAS / FnOS
**技术栈**: Shell Script + Docker + Alpine Linux

### 核心功能

通过 Docker 容器定时下载公开大文件来生成网络流量，并记录每日下载次数、流量和耗时统计。
...
| `USER_AGENT` | `traffic-keeper/2.6.16 curl/8.0` | User-Agent |
...
| `human_bytes` | `$1` (字节数) | 人类可读大小 (如 1.5GiB) | 字节数转可读格式 |
| `human_seconds` | `$1` (秒数) | `XXmin XXs` 格式 | 秒数转可读格式 |
...
```bash
nano /vol2/1000/Docker/traffic-keeper/.env
docker restart traffic-keeper
```
...
1. **使用场景**: 仅适合在拥有管理权限的个人 NAS 和网络环境中使用
2. **合理配置**: 请合理设置限速、休眠时间和每日流量上限，避免影响正常网络使用
3. **数据持久化**: 统计文件持久化保存到 `data/` 和 `流量统计/` 目录
4. **链接抓取**: 支持从 GitHub Release 和镜像站抓取下载链接
5. **故障恢复**: 抓取失败或抓取链接不可用时，下一轮自动重新抓取
