...
> **注意**：时间字段支持 `s`（秒）、`m`（分）、`h`（时）单位，如 `15m`、`1h`、`30s`；纯数字默认秒。数据字段支持 `K`、`M`、`G`、`T` 单位（1024进制），如 `1G`、`500M`、`10K`；纯数字默认字节。主脚本会自动统一转换为内部单位。
