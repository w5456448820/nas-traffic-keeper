#!/usr/bin/env bash
# =========================================================
#  Traffic Keeper - 远程部署辅助脚本
#  用法：将此脚本上传到 NAS 后执行：
#        bash deploy-remote.sh
#  说明：此脚本会自动下载所有必需文件并启动服务
# =========================================================
set -e

echo ""
echo "========================================"
echo "🚀 Traffic Keeper - 远程部署脚本"
echo "========================================"
echo ""
...
# ---------- 创建目录 ----------
mkdir -p "$PROJECT_DIR/data" "$PROJECT_DIR/流量统计" "$PROJECT_DIR/links"
cd "$PROJECT_DIR"
...
# ---------- 生成 traffic-keeper.sh ----------
echo "✅ 生成主运行脚本 (traffic-keeper.sh)"
cat > "$PROJECT_DIR/traffic-keeper.sh" <<'MAINSCRIPT'
#!/usr/bin/env sh
# =========================================================
...
chmod +x "$PROJECT_DIR/traffic-keeper.sh"

# ---------- 生成 fetch-links.sh ----------
echo "✅ 生成链接抓取脚本 (fetch-links.sh)"
cat > "$PROJECT_DIR/fetch-links.sh" <<'FETCHSCRIPT'
#!/usr/bin/env sh
# =========================================================
...
chmod +x "$PROJECT_DIR/fetch-links.sh"
...
echo "🔄 重启服务：cd $PROJECT_DIR && $DOCKER_COMPOSE restart"
echo "========================================"
